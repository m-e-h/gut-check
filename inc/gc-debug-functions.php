...
if ( class_exists( 'WP_Customize_Control' ) ) {
	class WP_Customize_Range extends WP_Customize_Control {
		...
		<label>
			...
			<?= "<input id='{$this->id}' type='range' class='{$this->id}' min='{$this->min}' max='{$this->max}' step='{$this->step}' value='{$this->value()}' oninput='{$this->id}_output.value = {$this->id}.value'"; ?><?php $this->link(); ?>>

			<br>

			<?= "<output id='{$this->id}_output' class='{$this->id}_output'>{$this->value()}</output>"; ?>
		</label>
			<?php
		}
	}
